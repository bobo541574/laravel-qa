...
<div class="d-flex flex-column vote-controls">
    ...
    <form id="up-vote-{{ $name .'-'. $model->id }}" action="{{ route($firstURISegment.'.vote', $model->id) }}" method="POST"
        style="display:none">
        @csrf
    ...
    <form id="down-vote-{{ $name .'-'. $model->id }}" action="{{ route($firstURISegment.'.vote', $model->id) }}"
